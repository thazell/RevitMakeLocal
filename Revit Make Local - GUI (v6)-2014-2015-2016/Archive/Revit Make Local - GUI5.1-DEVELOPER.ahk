...
;---### V5.1 release to correct Revit 2013 & 2014 dialog not dismissing (21st July 2013)
...
If VRSN <= 12																	; Since OneBox's arrival in version 2013, we have to check whether the user has this installed instead of separate discipline versions
{																				; Then we'll assemble the application path and Journal path accordingly
IniRead, RevitApp, %SettingsINI%, REVIT, %DSCPLN%%VRSN%
} Else {
IniRead, OneBox, %SettingsINI%, REVIT, %VRSN%OneBox
	if OneBox<>1
	{
	IniRead, RevitApp, %SettingsINI%, REVIT, %DSCPLN%%VRSN%
	}
	Else
	{
	IniRead, RevitApp, %SettingsINI%, REVIT, %VRSN%
	RevitApp = %RevitApp% 20%VRSN%\
	}
}

If VRSN <= 13																	; Autodesk changed their folder structure AGAIN in 2014!!
{
AppPath = %RevitApp%Program\
}
Else
{
AppPath = %RevitApp%\	
...
EnvGet, User, USERPROFILE
	if OneBox = 1
	{
	;using OneBox, no discipline designator in the path
	DSCPLNjourn =
	}
	Else
	{
	;not using OneBox, there are discipline designators in the path
	DSCPLNjourn .= DISCIPLINE A_Space
	}
JournPath = %user%\AppData\Local\Autodesk\Revit\Autodesk Revit %DSCPLNjourn%20%VRSN%\Journals\
}
...
; Now we create the journal so Revit will create a detached copy, no questions asked. We have 3 versions as things have changed in 2010 journals and then again in 2013 (detach and preserve or disable worksets)
;msgbox About to create detach journal in %JournPath%
If VRSN >= 10
{
	If VRSN <= 12
	{
		FileAppend,	;This works in 2010 to 2012
		(
		'
	...
	Else
	{
	;msgbox Detach option in 2013 or 2014
			FileAppend,	;This works in 2013 until Autodesk changes it yet again!
		(
		'
		Set Jrn = CrsJournalScript
		Jrn.Command "Internal" , "Open an existing project , ID_REVIT_FILE_OPEN"
...
; Now we create the journal so Revit will prompt us to open select worksets
	If VRSN >= 14
	{
		FileAppend,	;This works in 2014 until Autodesk changes things again!
		(
		'
		Set Jrn = CrsJournalScript
		Jrn.Command "Internal" , "Open an existing project , ID_REVIT_FILE_OPEN"
		Jrn.Data "File Name" , "IDOK", "%DESTINATION%%LOCALFILE%"    
		Jrn.Data "WorksetConfig" , "Custom", 1
		), %JournPath%select_ws.txt
	}
	Else	;This works in 2013 and before
	{
	FileAppend,
	(
	'
...
SplashCheck:
	...
	If VRSN >=14
	{
		If 3 <> DETACH							; We want to run the loop to check for the "central has been copied" warning when we OPEN local or use the Specify Worksets option in version 2014 and above.
		{
			Goto, Finishup
		}
		ExitApp
	...
	Else
	{
	If 3 = OPEN									;We only want to run the loop to check for the "central has been copied" warning when we OPEN local in version 2013 and below
		{
			Goto, Finishup
		}
		ExitApp
...
Else If VRSN >= 13
{
	IfWinExist, Copied Central Model
	{
   	WinClose
   	ExitApp
	}
